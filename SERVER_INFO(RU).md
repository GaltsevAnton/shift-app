...
- [ ] Доступ к аккаунту `hannoshift.notify@gmail.com` (Google Drive с бэкапами БД/фото/конфигов, Gmail для SMTP)
- [ ] Доступ к DuckDNS-аккаунту (для обновления IP нового сервера на домен)
- [ ] Git-репозиторий с исходным кодом (frontend/backend/report-service) должен быть доступен независимо от `hanno-web`
...
## 9) Docker — переход на контейнеризацию (dev, 2026-09-01)

**Статус**: полностью реализовано и протестировано на dev-окружении. Перенос на прод — запланирован, ещё не выполнен.

### Архитектура
Все 4 сервиса теперь собираются и запускаются через Docker, связаны единым `docker-compose.yml` в корне репозитория:

- **postgres** — официальный образ `postgres:17`, данные в named volume `hannoshift-pgdata`. На dev перенесены все 3 базы (`shiftapp`, `hanno_banquets`, `wordcards`), каждая со своим отдельным пользователем. Локальная Windows-установка PostgreSQL полностью удалена.
- **backend** — свой `backend/Dockerfile`, multi-stage build (Maven → JRE 21). Конфиг (`application.yml`) — часть кода/образа, без хардкода секретов (используются `${ПЕРЕМЕННАЯ:default}` плейсхолдеры). Секреты передаются через переменные окружения при запуске.
- **report-service** — свой `report-service/Dockerfile`, простой однослойный образ на `python:3.12-slim`.
- **frontend** — свой `frontend/Dockerfile`, multi-stage build (Node 20 → nginx:alpine). Внутри контейнера свой `nginx.conf`, адаптированный с прод-версии (`/api/`, `/photos/` проксируются на `backend:8080` по имени сервиса внутри Docker-сети, а не через `localhost`).

### Сеть между сервисами
Docker Compose создаёт общую внутреннюю сеть — сервисы видят друг друга по **имени сервиса**, не по `localhost`/IP:
- backend → postgres: `jdbc:postgresql://postgres:5432/shiftapp`
- backend → report-service: `http://report-service:8001`
- frontend (nginx) → backend: `http://backend:8080`

### Разбор частой ошибки — volume name mismatch
При `docker compose up`, если volume был создан вручную заранее (`docker volume create hannoshift-pgdata`), Compose по умолчанию добавляет префикс имени проекта и создаёт **новый, отдельный** volume (`shifts_app_hannoshift-pgdata`) вместо переиспользования существующего — из-за этого backend стартует с пустой базой. Решение — объявить volume как `external`:
```yaml
volumes:
  hannoshift-pgdata:
    external: true
    name: hannoshift-pgdata
```

### Повседневные команды

```bash
# запустить весь проект. Используешь, когда: поменял код и хочешь применить
# изменения, или запускаешь проект впервые (пересобирает образы + создаёт/пересоздаёт контейнеры)
docker compose up -d --build

# просто останавливает уже существующие, работающие контейнеры — как нажать паузу.
# Ничего не удаляет и не пересобирает, данные в volumes целы
docker compose stop

# запускает уже существующие, ранее остановленные контейнеры (обратное к stop).
# Ничего не собирает заново — быстрее, чем полный up --build
docker compose start
```

**Разница между `up -d --build`, `stop` и `start`**: `up --build` пересобирает образы из Dockerfile (если код менялся) и создаёт/пересоздаёт контейнеры — использовать после правок кода или при первом запуске. `stop`/`start` — просто выключить/включить уже существующие контейнеры без пересборки, как кнопка питания на уже собранном компьютере — использовать, когда код не менялся, а нужно просто временно освободить ресурсы ПК или снова продолжить работу.

```bash
# пересобрать конкретный сервис (не трогая остальные)
docker compose up -d --build backend
docker compose up -d --build frontend
docker compose up -d --build report-service

# выключить/включить конкретный сервис
...
docker compose logs -f backend    # live-режим

# полностью остановить и удалить контейнеры проекта (volumes НЕ трогает)
docker compose down

# посмотреть, сколько места занимают образы/контейнеры/volumes/кэш (без удаления)
docker system df

# почистить неиспользуемые образы/контейнеры/build-кэш (volumes НЕ трогает)
docker system prune

# почистить неиспользуемые volumes отдельно (осторожно — реально удаляет данные,
# но только те volumes, что не привязаны ни к одному контейнеру)
docker volume prune
```

### Известные подводные камни, с которыми уже столкнулись
- **Регистр букв в путях импортов** (`SettingsPage.jsx` vs `Settingspage.jsx`) — на Windows работает молча, Linux-контейнер (где идёт сборка) чувствителен к регистру и падает с `Could not resolve`. Переименовывать файлы через `git mv` в два шага (временное имя → нужное), обычный `mv`/переименование в Проводнике иногда не фиксирует смену регистра.
- **CORS**: при добавлении нового адреса, с которого открывается фронт (например, `http://localhost:8888` для Docker-версии на dev), нужно добавить его в `allowedOrigins` в `SecurityConfig.java` и пересобрать backend.
- **Git Bash на Windows**: не наследует переменные окружения, заданные через `setx` в Windows — для тестовых `docker run` команд с паролями нужно либо вписывать значение прямо в команду, либо использовать CMD/PowerShell.
- **`host.docker.internal`**: специальный DNS-адрес Docker Desktop для обращения из контейнера к процессам на хост-машине (использовался до перехода PostgreSQL в сам Docker).

### Следующие шаги (не выполнено)
- [ ] Перенос этой же архитектуры на прод (`hanno-web`) — перенести реальные прод-данные в volume, продумать связку с внешним nginx/SSL (Certbot), спланировать минимальный простой при переключении
- [ ] Файл `.env` для секретов в docker-compose (сейчас частично передаются через переменные окружения хоста напрямую) — унифицировать
- [ ] Обновить план аварийного восстановления (§7) под Docker-архитектуру — после перехода прода на Docker он станет значительно короче (`git clone` + восстановить данные + `docker compose up`)

---

## 10) Известные пробелы / что стоит доделать
